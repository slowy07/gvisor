...
// a local raw socket and other non-raw IP based socket.
TEST(RawSocketTest, SendAndReceiveOnRawAndDgramSocket) {
  SKIP_IF(!ASSERT_NO_ERRNO_AND_VALUE(HaveRawIPSocketCapability()));

  constexpr char payload[] = "soweoneul malhaebwa";
  ...
  char recv_payload[sizeof(payload) + 1] = {};
  ASSERT_THAT(
